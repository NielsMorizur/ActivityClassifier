...
# opens an explorer window where a csv file can be selected
def read_csv_file():
...
# called when button pressed, calls read_csv_file() and then the main function plot_graph()
def select_file():
    global testingData
    # sets a global variable to be the dataset from the input file
    testingData = read_csv_file()
    plot_graph()

# function that contains the original code plus plotting of the graph and outputting a csv file
def plot_graph():
    ...
    # normalizes the features so that it has a mean of 0 and Standard Deviation of 1 by using z-score normalization
    ...
    # use the classifier on the input csv file
    features_df_test = pd.DataFrame(extracting(testingData))
    ...
    # convert output classification array to a DataFrame so it can be exported to a csv file
    y_pred = pd.DataFrame(y_pred)
    print(y_pred)
    y_pred.to_csv('Classification of input file.csv', index=False)

    # plotting line graph
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(y_pred.index, y_pred.iloc[:, 0], marker='o', linestyle='-', color='b')
    ax.set_xlabel('5 Second shuffle periods')
    ax.set_ylabel('Classification (0 or 1)')
    ax.set_title('Classifying app, 0=walking 1=jumping')
    ax.grid(True)

    # plot the line graph in the same window that was already open
...
def run_app():
    ...
    root.title("Input Accelerometer Data")

    # stop running this program when the window is closed
    def on_closing():
        exit()

    root.protocol("WM_DELETE_WINDOW", on_closing)

    # create a button that allows the user to select a csv file by calling select_file()
    select_button = tk.Button(root, text="Select CSV File", command=select_file)
    select_button.pack(pady=20)
    root.mainloop()

# start a thread with the tkinter window so the rest of the code can run while it is open
